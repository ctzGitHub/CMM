...
class ResNet1d(nn.Sequential):
    def __init__(self, block, layers, kernel_size=5, num_classes=64, input_channels=12):
...
if __name__ == '__main__':
    input1 = torch.randn(8, 12, 8000)
    # input2 = torch.randn(8, 16, 12, 56, 56)

    # input1 = input1.to(device)
    # input2 = input2.to(device)

    SE_ECGNet = ResNet1d(BasicBlock1d, [1, 1, 1])
    # SE_ECGNet = SE_ECGNet.cuda()
    # with torch.no_grad():

    # H = [SE_ECGNet(input1[:, i, :, :]).view((input1.shape[0], -1, 1)) for i in range(config.n_segments+1)]
